...
def process_file(cpu_count: int, start_end: list):
    """Process data file by distributing chunks across multiple CPUs using concurrent.futures."""
    result = dict()  # Dictionary to store combined results
    
    # Use ProcessPoolExecutor for parallel processing
    with ProcessPoolExecutor(max_workers=cpu_count) as executor:
        # Submit tasks to the executor for each chunk and collect the results
                
        gc_disable()
        futures = [executor.submit(_process_file_chunk, *chunk_info) for chunk_info in start_end]
        gc_enable() 
        
        for future in futures:
            chunk_result = future.result()  # Get the result from the future
            
            # Combine all results from each chunk
